...
import { getUsageById } from 'apiSdk/usages';
import { UsageInterface } from 'interfaces/usage';

function UsageViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('usage', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/usages/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Usage Date"
                  text={data?.usage_date ? format(parseISO(data?.usage_date as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Usage Time"
                  text={data?.usage_time ? format(parseISO(data?.usage_time as unknown as string), 'dd-MM-yyyy') : ''}
                />
