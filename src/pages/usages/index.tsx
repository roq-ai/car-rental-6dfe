...
import { getUsages, deleteUsageById } from 'apiSdk/usages';
import { UsageInterface } from 'interfaces/usage';

type ColumnType = ColumnDef<UsageInterface, unknown>;

interface UsageListPageProps {
...
export function UsageListPage(props: UsageListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UsageInterface>>(
    () => `/usages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UsageInterface) => {
    if (hasAccess('usage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/usages/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.usage_date ? format(parseISO(record?.usage_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'usage_time',
      header: 'Usage Time',
      accessorKey: 'usage_time',
      cell: ({ row: { original: record } }: any) =>
        record?.usage_time ? format(parseISO(record?.usage_time as unknown as string), 'dd-MM-yyyy') : '',
    },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/usages/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('usage', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/usages/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('usage', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="usage" />
        </Flex>

        {hasAccess('usage', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/usages/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
